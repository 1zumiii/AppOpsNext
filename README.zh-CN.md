...
| 设置与诊断 | 支持简体中文、英文和跟随系统语言，提供连接状态与诊断报告，并静默检查是否有新版本。 |
| 实验性功能 | 监测指定应用的指定权限并在被使用时通知。「应用 + 权限」构成一个监测点，每个监测点可单独设置通报间隔、提醒方式、通报内容，以及是否只在应用不在屏幕上时通报。默认不监测任何内容，开启时通过自检判断当前设备是否支持。 |
...
| 权限 | 用途 |
| --- | --- |
| `QUERY_ALL_PACKAGES` | AppOps 管理器必须发现构建时未知的应用包。 |
| `INTERNET` | 仅用于静默检查是否存在新版本。 |
| `POST_NOTIFICATIONS` | 新装应用自动套用模板的结果，以及实验性监测功能的访问提醒。 |
| `FOREGROUND_SERVICE`、`FOREGROUND_SERVICE_SPECIAL_USE` | 实验性后台监测开启期间，保持其回调可达。 |

更新检查只读取一个 GitHub 公开接口，不做其他任何请求。它不会强制更新、不会下载或
安装任何内容，也不会为此弹窗或发通知：有新版本时只在设置页的「App 版本」条目旁显示，
除此之外不出现在任何地方。**不会上传任何用户数据。**

诊断报告不会被自动发送到任何地方。它在设备本地生成，只有你主动操作时才复制到剪贴板，
是否分享完全由你决定。
...
- **历史记录较旧或不完整：** 检查保存时间，尝试手动刷新并确认后端连接。
  可读取哪些记录由 Android 决定。
- **后台监测没有任何提示：** 该功能为实验性，依赖并非所有 ROM 都提供的特权接口。
  重新开启一次并查看自检结果，它会区分「设备不支持」和「部分监听未注册成功」。
  另外请确认已选择至少一个应用及其权限，默认不监测任何内容。
- **监测计数与应用调用次数不同：** 计数表示被通报的次数。Android 可能为一次调用
  回报多个事件；如果该监测点设置了通报间隔，间隔之内的访问则完全不会被通报。
  仅统计当前用户空间内被选中的应用。监测结果不是精确的 API 调用审计日志。
- **高频权限把通知刷满：** 定位这类权限每送达一次定位结果就上报一次，不设间隔地
  监测会变成每秒一条。请在「设置 → 实验性功能 → 监测设置 → 监测点设置」里给
  该监测点设置通报间隔。
- **开了「仅在应用不在屏幕上时通报」，看得见的访问还是被通报：** 判定依据是 AppOps
  为该应用记录的进程状态，只有应用正显示在屏幕上才算前台，前台服务算作不在屏幕上。
  如果这个状态完全读不到，访问会照常通报而不是被丢弃——监测不会因为查不到就静默。
- **强行停止后监测失效：** 强行停止会移除其服务，并使系统不再自动重启它。
  开关保持开启的前提下，重新打开 AppOpsNext 即可恢复。
- **熄屏一段时间后监测停止：** 部分厂商的电池优化或后台管理会结束本应用进程，
  前台服务也不能幸免。这不受应用控制。若需长时间运行该功能，请在系统设置中将
  AppOpsNext 排除出电池优化，并在最近任务中锁定。重新打开应用会自动恢复监测。
...
| `templates/`、`newapps/`、`batch/` | 模板持久化、新装应用检测、可恢复的规则执行和批量目标。 |
| `history/` | 系统历史解析、刷新调度和本地快照。 |
| `diagnostics/` | 环境与连接报告。 |
| `monitor/` | 实验性访问监测：经转发的 binder 调用注册监听、监测点级通报设置、事件过滤与通知。 |
| `update/` | 通过 GitHub API 检查版本与版本号比较。 |
